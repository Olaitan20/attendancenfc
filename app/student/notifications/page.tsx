    ...
    <DashboardShell title="Notifications">
      ...
      <div className="max-w-2xl flex flex-col gap-4">
        {unread.length > 0 && (
          <Card>
            <CardHead>
              ...
              <span className="w-5 h-5 rounded-full bg-danger text-white text-[10px] font-bold flex items-center justify-center font-mono">
                {unread.length}
              </span>
            </CardHead>
            {unread.map((n, i) => (
        ...
        {read.length > 0 && (
          <Card>
            <CardHead>
              <p className="text-[13.5px] font-bold text-muted">Earlier</p>
            </CardHead>
            {read.map((n, i) => (
              <div key={i} className="flex gap-3 px-5 py-4 border-b border-border1 last:border-0 hover:bg-surface2/40 transition-colors cursor-pointer">
